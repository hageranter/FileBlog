...
function getSlugFromPath() {
  ...
  return parts.length > 1 ? parts[1] : null; // /post/<slug>
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (token && currentUsername && commentAvatarEl) {
    fetch(`/users/${currentUsername}`)
      .then(res => res.json())
      .then(user => {
        commentAvatarEl.src = user.avatarUrl || "/images/profile-icon.jpg";
      })
      .catch(() => {
        commentAvatarEl.src = "/images/profile-icon.jpg";
      });
  }
  setupEditorSendButton();
});
...
function confirmDeletePost() {
  ...
  fetch(`/posts/${currentSlug}`, {
    ...
    .then(res => {
      ...
      Swal.fire({ icon: "success", title: "Post deleted", text: "The post was successfully deleted." })
        .then(() => { window.location.href = "/posts"; });
    })
    .catch(() => {
      Swal.fire({ icon: "error", title: "Error", text: "Failed to delete the post." });
    });
}
